...
class StmHistogram implements Histogram {
  ...
  public int[] getBins() {
      return atomic(() -> {
          int count = 0;
          final int[] tmp = new int[counts.length];
          for (TxnInteger txn : counts) 
              tmp[count++] = txn.get(); 
          return tmp;
      });
  }
  ...
  public void transferBins(final Histogram hist) {
      int count = 0;
      for (TxnInteger txn : counts) {
          final int current = count++;
          atomic(() -> txn.increment(hist.getAndClear(current)));
      }
  }
}
