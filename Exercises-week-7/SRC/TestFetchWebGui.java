...
public class TestFetchWebGui {
  ...
  private static void goodFetch() {
    ...
    int count = 0;
    DownloadWorkerN.problemSize = urls.length;
    final JProgressBar progressBar = new JProgressBar(0, 100);
    final DownloadWorkerN[] workers = new DownloadWorkerN[urls.length];
    for (String url : urls) { 
	    workers[count++] = new DownloadWorkerN(textArea, url, count);
	    workers[count - 1].addPropertyChangeListener( evt -> {
    ...
    fetchButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          for (DownloadWorkerN task : workers)
		  task.execute();
        }});
    ...
    cancelButton.addActionListener(e -> {
		for (DownloadWorkerN downloadTask : workers)
			if (downloadTask.getState() != SwingWorker.StateValue.DONE)
 ...
 static class DownloadWorkerN extends SwingWorker<String, String> {
	 ...
	 public DownloadWorkerN(TextArea textArea, String url, int index) {
		 this.url = url;
		 this.textArea = textArea;
		 this.index = index;
	 }
	 ...
	 public String doInBackground() {
		 ...
		 setProgress(100 * count / problemSize);
		 return result; 
	 }
	 ...
	 public void done() {
		 try {
		   ...
		   catch (CancellationException exn) { textArea.append("Download canceled for: "+ this.url+ "\n"); }  // (3)
	 }
 }
